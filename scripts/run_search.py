#!/usr/bin/env python3
"""
Simple script to run a flight search from the command line.

Usage:
    python scripts/run_search.py "New York" "Los Angeles" 2025-03-01
    python scripts/run_search.py "New York" "Los Angeles" 2025-03-01 2025-03-08
...
# This lets us import from src/ regardless of where we run the script from
...
async def main():
    ...
    if len(sys.argv) < 4:
        ...
        print("\nExample:")
        print('  python scripts/run_search.py "New York" "Los Angeles" 2025-03-01')
        sys.exit(1)
    ...
    flights = await search_google_flights(
        ...
        headless=True  # Run without visible browser (required for cloud/servers)
    )
