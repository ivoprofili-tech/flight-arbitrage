...
Usage:
    python scripts/run_kayak.py "New York" "Los Angeles" 2025-03-01
    python scripts/run_kayak.py "New York" "Los Angeles" 2025-03-01 2025-03-08
...
from src.scraper import search_kayak_flights, save_results_to_file
...
async def main():
    if len(sys.argv) < 4:
        ...
        print("\nExample:")
        print('  python scripts/run_kayak.py "New York" "Los Angeles" 2025-03-01')
        sys.exit(1)
    ...
    if flights:
        ...
        filename = save_results_to_file(
            ...
            departure_date=departure_date,
            return_date=return_date,
            filename=f"kayak_{origin.replace(' ', '_')}_to_{destination.replace(' ', '_')}.txt"
        )
